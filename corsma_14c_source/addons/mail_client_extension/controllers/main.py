...
class MailClientExtensionController(http.Controller):
    ...
    def auth(self, **values):
        """
         Once authenticated this route renders the view that shows an app wants to access Corsma.
    ...
    def auth_confirm(self, scope, friendlyname, redirect, info=None, do=None, **kw):
        """
        Called by the `app_auth` template. If the user decided to allow the app to access Corsma, a temporary auth code
