...
QUnit.test('CorsmaBot initialized at init', async function (assert) {
    // TODO this test should be completed in combination with
    // implementing _mockMailChannelInitCorsmaBot task-2300480
    assert.expect(2);
    ...
    assert.verifySteps(
        ['init_odoobot'],
        "should have initialized CorsmaBot at init"
    );
});
