...
class GoogleEvent(abc.Set):
    ...
    Inspired by Corsma recordset, one instance can be a single Google event or a
    ...
    def _meta_odoo_id(self, dbname):
        """Returns the Corsma id stored in the Google Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env, model):
        ...
        # Hence, we may have two Google recurrences linked to the same Corsma id.
        # Therefore, we only consider Corsma records without google id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Corsma.
        ...
        # (he is the organizer in Google). The "real" owner (in Corsma) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Corsma several users might have the same email :/
