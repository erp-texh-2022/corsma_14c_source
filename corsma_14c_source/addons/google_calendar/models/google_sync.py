...
# This ensures changes are sent to Google only if they really happened in the Corsma database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Corsma.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Corsma DB (_sync_google2odoo).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Corsma. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Corsma
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Corsma to Google
    ...
    def _odoo_values(self, google_event: GoogleEvent, default_reminders=()):
        """Implements this method to return a dict of Corsma values corresponding
        to the Google event given as parameter
        :return: dict of Corsma formatted values
        """
    ...
    def _notify_attendees(self):
        ...
        At the initialization of a synced calendar, Corsma requests all events for a specific
