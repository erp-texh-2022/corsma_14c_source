...
class CorsmaTestTimeoutError(Exception):
    pass


class CorsmaTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise CorsmaTestError("Installation of package failed")
        else:
        ...
        raise CorsmaTestError("Package test: FAILED. Not able to install base.")
...
class Docker():
    ...
    def test_odoo(self):
        logging.info('Starting to test Corsma install test')
        ...
        if self.is_running():
            self.stop()
            raise CorsmaTestTimeoutError('Corsma pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise CorsmaTestError('Error while installing/starting Corsma after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        logging.info("Syncing Corsma files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Corsma %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Corsma {sv}"/python/python.exe \'c:\\Program Files\\Corsma {sv}\\server\\odoo-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
