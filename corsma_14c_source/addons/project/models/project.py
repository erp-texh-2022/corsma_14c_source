...
class Task(models.Model):
    ...
    def _notify_email_header_dict(self):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Corsma-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Corsma-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Corsma-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
