...
class WebRequest(object):
    """ Parent class for all Corsma Web request types, mostly deals with
    ...
    @lazy_property
    def session(self):
        """ :class:`CorsmaERPSession` holding the HTTP session data for the
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Corsma implements token-based `CSRF protection
        ...
        * if the endpoint can be called by external parties (not from
          Corsma) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Corsma Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Corsma Session Expired"
...
Corsma URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.groupecorsma.com/documentation/14.0/developer/reference/http.html#csrf for
more details.

* if this endpoint is accessed through Corsma via py-QWeb form, embed a CSRF
...
class CorsmaERPSession(sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(CorsmaERPSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ CorsmaERP provides languages which may not make sense and/or may not
...
class Root(object):
    """Root WSGI application for the CorsmaERP Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        ...
        return sessions.FilesystemSessionStore(
            path, session_class=CorsmaERPSession, renew_missing=True)
...
def db_filter(dbs, httprequest=None):
    ...
    if odoo.tools.config['dbfilter']:
        ...
        # In case --db-filter is not provided and --database is passed, Corsma will
