...
@tagged("odoobot")
class TestCorsmabot(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestCorsmabot, cls).setUpClass()
        ...
        cls.odoobot_ping_body = '<a href="http://groupecorsma.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@CorsmaBot</a>' % (cls.odoobot.id, cls.odoobot.id)
    ...
    def test_odoobot_ping(self):
        ...
        # Corsmabot should not be a follower but user_employee and user_admin should
