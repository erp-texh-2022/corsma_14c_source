...
# This ensures changes are sent to Microsoft only if they really happened in the Corsma database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Corsma.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_microsoft2odoo(self, microsoft_events: MicrosoftEvent, default_reminders=()):
        """Synchronize Microsoft recurrences in Corsma. Creates new recurrences, updates
        existing ones.
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Corsma to Microsoft
