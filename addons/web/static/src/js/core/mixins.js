...
function CorsmaEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
CorsmaEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

CorsmaEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * CorsmaERP Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new CorsmaEvent(this, name, info);
