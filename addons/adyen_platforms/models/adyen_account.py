...
class AdyenAccount(models.Model):
    ...
    def create(self, values):
        ...
        # Create account on groupecorsma.com, proxy and Adyen
        response = adyen_account_id._adyen_rpc('create_account_holder', adyen_account_id._format_data())

        # Save adyen_uuid and proxy_token, that have been generated by groupecorsma.com and the proxy
    ...
    def action_create_redirect(self):
        ...
        The action will redirect the user to accounts.groupecorsma.com to link an Corsma user_id to the Adyen
        account. After logging in on groupecorsma.com the user will be redirected to his DB with a token in
    ...
    def _update_kyc_status(self, checks):
        ...
        if kyc_status_message.decode() != self.kyc_status_message:
            self.sudo().message_post(body = kyc_status_message, subtype_xmlid="mail.mt_comment") # Message from Corsma Bot
