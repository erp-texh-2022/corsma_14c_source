 ...
 * consumed by CorsmaERP's RPC APIs.
 *
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of CorsmaERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the CorsmaERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // CorsmaERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // CorsmaERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, CorsmaERP domain
     ...
     * an SQL-like form usable by CorsmaERP's ORM.
     *
     ...
     * Corsma's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    ...
    @param {String} name name of the CorsmaERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound CorsmaERP model.
     *
