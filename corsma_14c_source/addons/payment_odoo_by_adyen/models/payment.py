...
from odoo.addons.payment_odoo_by_adyen.controllers.main import CorsmaByAdyenController

_logger = logging.getLogger(__name__)


class AcquirerCorsmaByAdyen(models.Model):
    _inherit = 'payment.acquirer'

    provider = fields.Selection(selection_add=[
       ('odoo_adyen', 'Corsma Payments by Adyen')
    ...
    def _check_odoo_adyen_test(self):
        for payment_acquirer in self:
            if payment_acquirer.provider == 'odoo_adyen' and payment_acquirer.state == 'test':
                raise ValidationError(_('Corsma Payments by Adyen is not available in test mode.'))

    def _get_feature_support(self):
        res = super(AcquirerCorsmaByAdyen, self)._get_feature_support()
    ...
    def odoo_adyen_form_generate_values(self, values):
        ...
        data = {
            ...
            'metadata': {
                ...
                'notification_url': urls.url_join(base_url, CorsmaByAdyenController._notification_url),
            },
...
class TxCorsmaByAdyen(models.Model):
    ...
    def _odoo_adyen_form_get_tx_from_data(self, data):
        ...
        if not reference:
            error_msg = _('Corsma Payments by Adyen: received data with missing reference (%s)', reference)
            _logger.info(error_msg)
            raise ValidationError(error_msg)

        tx = self.env['payment.transaction'].search([('reference', '=', reference)])
        if not tx or len(tx) > 1:
            error_msg = _('Corsma Payments by Adyen: received data for reference %s') % (reference)
            if not tx:
    ...
    def _odoo_adyen_form_validate(self, data):
        ...
        if data['success']:
            ...
            error = _('Corsma Payment by Adyen: feedback error')
            _logger.info(error)
