    ...
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'CorsmaTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Corsma DB and
...
def check_image():
    ...
    """
    url = 'https://nightly.groupecorsma.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def load_certificate():
    """
    Send a request to Corsma with customer db_uuid and enterprise_code to get a true certificate
    """
    ...
    if db_uuid and enterprise_code:
        url = 'https://www.groupecorsma.com/odoo-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Corsma server
    """
    server = get_odoo_server_url()
    if server:
        urllib3.disable_warnings()
