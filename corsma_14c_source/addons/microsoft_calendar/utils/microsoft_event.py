...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Corsma recordset, one instance can be a single Microsoft event or a
    ...
    def _meta_odoo_id(self, microsoft_guid):
        """Returns the Corsma id stored in the Microsoft Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env):
        ...
        # Hence, we may have two Microsoft recurrences linked to the same Corsma id.
        # Therefore, we only consider Corsma records without microsoft id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Microsoft and Corsma.
        # Let userA, userB be two new users (never synced to Microsoft before).
        # UserA creates an event in Corsma (he is the owner) but userB syncs first.
        ...
        # (he is the orginizer in Microsoft). The "real" owner (in Corsma) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Microsoft: 1 email = 1 user; but in Corsma several users might have the same email
