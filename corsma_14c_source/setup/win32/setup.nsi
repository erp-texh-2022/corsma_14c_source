...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Corsma ERP ${VERSION}"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.groupecorsma.com/page/contactus"
...
; English
LangString DESC_Corsma_Server ${LANG_ENGLISH} "Install the Corsma ERP with all the Corsma standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Corsma."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Corsma for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Corsma ERP"
...
LangString TITLE_Corsma_Server ${LANG_ENGLISH} "Corsma ERP"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Corsma"

; French
LangString DESC_Corsma_Server ${LANG_FRENCH} "Installation du Serveur Corsma avec tous les modules Corsma standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Corsma."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Corsma pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Corsma"
...
LangString TITLE_Corsma_Server ${LANG_FRENCH} "Serveur Corsma"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Corsma"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_Corsma_Server) SectionCorsma_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the Corsma ERP, keep the login/password from the config file
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Corsma" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@groupecorsma.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.groupecorsma.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@groupecorsma.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionCorsma_Server} $(DESC_Corsma_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        ${If} ${RunningX64}
            StrCpy $INSTDIR "$PROGRAMFILES64\Corsma ${VERSION}"
        ${Else}
            StrCpy $INSTDIR "$PROGRAMFILES\Corsma ${VERSION}"
        ${EndIf}
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionCorsma_Server} $0
